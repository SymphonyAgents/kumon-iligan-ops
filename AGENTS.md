...
frontend/
  ...
  lib/utils.ts             # cn(), formatPeso(), formatDate(), etc.
...
- **Migrations**: `pnpm db:generate` creates SQL files in `backend/migrations/`. `pnpm db:migrate` runs them. `pnpm db:push` pushes schema directly (dev only).
...
4. If the feature needs a new table, add it to `backend/src/db/schema.ts` and run `pnpm db:generate` + `pnpm db:migrate`

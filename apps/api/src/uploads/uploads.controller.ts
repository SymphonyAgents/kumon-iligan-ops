...
export class UploadsController {
  ...
  async createPresignedUrl(@Body() dto: PresignedUrlDto, @Req() req: AuthedRequest) {
    // Verify user has branch-level access to the payment they're uploading for
    const user = await this.usersService.findById(req.user.id);
    if (user && user.userType !== 'superadmin') {
      const txnBranchId = await this.uploadsService.getReceiptBranchId(dto.txnId);
      if (txnBranchId !== null && user.branchId !== null && txnBranchId !== user.branchId) {
        throw new ForbiddenException('You do not have access to upload files for this payment.');
      }
    }

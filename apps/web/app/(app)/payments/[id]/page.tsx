...
export default function PaymentDetailPage({ params }: { params: Promise<{ id: string }> }) {
 ...
 const { data: payment, isLoading, isError } = usePaymentQuery(id) as {
 ...
 const flagMut = useFlagPaymentMutation(() => { setFlagOpen(false); setFlagNote(''); });
 const rejectMut = useRejectPaymentMutation(() => { setRejectOpen(false); setRejectNote(''); });
 const replyMut = useReplyPaymentMutation(() => setReply(''));
 const deleteMut = useDeletePaymentMutation(() => { setShowDelete(false); router.push('/payments'); });
 ...
 {verifierName ?? 'Admin'} <span className="text-muted-foreground font-normal">(admin)</span>
 </p>
 <p className="text-xs text-muted-foreground">flagged {relativeTime(payment.verifiedAt)}</p>
 </div>
 </div>
 {payment.note && (
 ...
 Awaiting reply from {payment.recordedByUser?.fullName ? toTitleCase(payment.recordedByUser.fullName) : 'recorder'}.
 </p>
 )}
 </div>
 )}
 ...
 <DetailRow
 label="Expected"
 value={fmtPeso(payment.expectedAmountSnapshot)}
 muted
 />
 <div className="h-px bg-border my-1" />
 <DetailRow label="Reference" value={payment.referenceNumber ?? '—'} mono />
 <DetailRow label="Method" value={PAYMENT_METHOD_LABELS[payment.paymentMethod] ?? payment.paymentMethod} />
 ...
 <Dialog open={flagOpen} onOpenChange={(v) => { if (!v) { setFlagOpen(false); setFlagNote(''); } }}>
 ...
 <DialogFooter>
 <Button variant="secondary" onClick={() => { setFlagOpen(false); setFlagNote(''); }}>Cancel</Button>
 <Button
 variant="danger"
 ...
 <Dialog open={rejectOpen} onOpenChange={(v) => { if (!v) { setRejectOpen(false); setRejectNote(''); } }}>
 <DialogContent className="max-w-sm">
 <DialogHeader>
 ...
 <DialogFooter>
 <Button variant="secondary" onClick={() => { setRejectOpen(false); setRejectNote(''); }}>Cancel</Button>
 <Button
 variant="danger"

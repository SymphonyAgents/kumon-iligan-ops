...
export default function ReportsPage() {
 ...
 <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 mb-6">
 <KPICard icon={CurrencyCircleDollarIcon} label={`Collected (${MONTHS[month - 1]})`} value={fmt(stats.verifiedThisMonth)} sub="verified payments"accent="bg-status-paid-dot" />
 ...
 <KPICard icon={WarningCircleIcon} label="Unpaid / overdue"value={stats.overdue} sub={`${MONTHS[month - 1]} ${year}`} accent={stats.overdue > 0 ? 'bg-err' : undefined} />
 </div>
 )}
 ...
 <Link href="/payments" className="flex items-center gap-1 text-xs font-semibold text-warn hover:underline shrink-0">
 ...
 <p className="text-xs text-muted-foreground truncate">{s.guardianName} · {s.level ?? 'No level'}</p>
 </div>
 <div className="text-right shrink-0">
 {period
 ? <><StatusBadge status={period.status} /><p className="text-xs text-muted-foreground mt-0.5">{fmt(period.expectedAmount - period.paidAmount)} due</p></>
 ...
 {unpaid.length > 12 && (
 <div className="px-4 py-3 bg-secondary/40 text-xs text-muted-foreground text-center">+{unpaid.length - 12} more</div>
 )}
 </div>
 )}
 </div>

...
export default function StaffProfilePage() {
  ...
  const approveMut = useApproveUserMutation(() => {
    setApproveOpen(false);
    router.push('/staff?tab=pending');
  });
  const rejectMut = useRejectUserMutation(() => {
    setRejectOpen(false);
    router.push('/staff?tab=pending');
  });
  ...
  const errors = {
    contactNumber:
      touched.contactNumber && form.contactNumber && !PHONE_RE.test(form.contactNumber)
        ...
        : undefined,
    birthday: touched.birthday && !isValidDate(form.birthday) ? 'Enter a valid date' : undefined,
  };
        ...
        <div className="mb-6 flex items-center gap-3 p-4 bg-warn-soft dark:bg-amber-950 border border-warn/40 dark:border-amber-800 rounded-lg">
          <div className="flex-1">
            ...
            <p className="text-xs text-warn mt-0.5">
              Approve or reject their access to the system.
            </p>
          </div>

...
} from '@/components/users/UserRoleConfirmDialog';
import {
  UserBranchConfirmDialog,
  type PendingBranchChange,
} from '@/components/users/UserBranchConfirmDialog';
...
function StaffContent() {
  ...
  const renderUserCard = (u: AppUser): DataCardProps => {
    ...
    const branchLabel = u.branchId
      ? toTitleCase(branchNameById.get(u.branchId) ?? '')
      : 'No branch';
    return {
      title: primary,
      description: u.email,
  ...
  return (
    <>
      ...
      <ConfirmDialog
        ...
        onConfirm={() => {
          if (approveTarget) approveMut.mutate(approveTarget.id);
        }}
      ...
      <ConfirmDialog
        ...
        confirmLabel="Reject"
        onConfirm={() => {
          if (rejectTarget) rejectMut.mutate(rejectTarget.id);
        }}
      ...
      />
      <div>
        <PageHeader title="Users" subtitle="Manage team roles and access" />
        ...
        <div className="flex gap-1 border-b border-border mb-6">
          <button
            ...
            className={cn(
              ...
              tab === 'approved'
                ? 'text-foreground'
                : 'text-muted-foreground hover:text-foreground dark:hover:text-muted-foreground/60',
            )}
          >
            Approved
            {tab === 'approved' && (
              <span className="absolute bottom-0 left-0 right-0 h-0.5 bg-secondary dark:bg-card rounded-t" />
            )}
          </button>
          <button
          ...
          >
            <span className="flex items-center gap-1.5">
              Pending
              {pendingUsers.length > 0 && <span className="w-2 h-2 rounded-full bg-err" />}
            </span>
            {tab === 'pending' && (
              <span className="absolute bottom-0 left-0 right-0 h-0.5 bg-secondary dark:bg-card rounded-t" />
            )}

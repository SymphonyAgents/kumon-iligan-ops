    ...
    <div>
      ...
      <div className="flex flex-wrap items-center gap-2 mb-4">
        ...
        <Select
          ...
          onValueChange={(v) => setPerformedByStr(v === 'all' ? '' : v)}
        >
      ...
      <DataTable
        ...
        renderMobileCard={(entry) => {
          const eventLabel =
            entry.auditType && AUDIT_TYPE_LABELS[entry.auditType]
              ? AUDIT_TYPE_LABELS[entry.auditType]
              : entry.action.replace(/_/g, ' ').replace(/\b\w/g, (c) => c.toUpperCase());
          const eventStyle =
            entry.auditType && AUDIT_TYPE_STYLES[entry.auditType]
          ...
          return {
            ...
            description: entry.entityId ? (
              <span className="font-mono">#{entry.entityId}</span>
            ) : undefined,
            badge: (
              <span
                className={`inline-flex items-center px-2 py-0.5 rounded text-xs font-medium whitespace-nowrap ${eventStyle}`}
              >
                {eventLabel}
              </span>
            ),

...
export default function LoginPage() {
  ...
  function handleGoogleLogin() {
    setLoading(true);
    setError(null);
    signIn('google', { callbackUrl: '/' }).catch((err: Error) => {
    ...
    <main className="flex min-h-screen flex-col items-center justify-center bg-secondary">
      <div className="flex flex-col items-center gap-8">
        <div className="flex flex-col items-center gap-2 text-center">
          ...
          <p className="text-sm text-muted-foreground">Sign in to continue to Kumon Iligan</p>
        </div>
        {error && <p className="text-sm text-err text-center max-w-xs">{error}</p>}
        <button
          onClick={handleGoogleLogin}
          disabled={loading}
          className="flex items-center gap-3 px-5 py-2.5 bg-secondary text-white text-sm font-medium rounded-lg hover:bg-secondary active:scale-[0.98] transition-all duration-150 cursor-pointer disabled:opacity-60 disabled:cursor-not-allowed"
        >
          <svg
            width="18"
            height="18"

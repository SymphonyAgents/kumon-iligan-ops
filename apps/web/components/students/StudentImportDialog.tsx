...
import { CloudArrowUpIcon, FileCsvIcon, CheckCircleIcon, WarningCircleIcon, XIcon } from '@phosphor-icons/react';
...
function parseRow(raw: Record<string, string>): { ok: true; row: CsvRow } | { ok: false; reason: string } {
  ...
  const guardianName = pickHeader(raw, ['guardianname', 'guardian', 'parent', 'parentname'])?.trim();
  const guardianPhone = pickHeader(raw, ['guardianphone', 'phone', 'parentphone', 'contact'])?.trim();
  const level = pickHeader(raw, ['level', 'kumonlevel'])?.trim() || undefined;
  const enrollmentDate = pickHeader(raw, ['enrollmentdate', 'enrolled', 'startdate'])?.trim() || undefined;
          ...
          {!result && (
            <div className="rounded-xl border border-border bg-secondary/40 p-4">
              ...
              <p className="text-[11.5px] text-muted-foreground mt-3 leading-relaxed">
                Families are matched by guardian phone — duplicates are reused, new ones are auto-created.
                If <span className="font-mono">enrollmentDate</span> is blank, today&apos;s date is used.
              </p>
              <button
                type="button"
